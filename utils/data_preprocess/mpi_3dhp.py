...
def train_data(dataset_path, out_path, joints_idx, scaleFactor, extract_img=False, fits_3d=None):
    ...
    for user_i in tqdm(user_list):
        for seq_i in tqdm(seq_list):
            ...
            for j, vid_i in tqdm(enumerate(vid_list), total=len(vid_list)):
                ...
                if extract_img:
                    ...
                    frame_num = len(glob.glob(os.path.join(imgs_path, '*.jpg')))
                ...
                for i, img_i in tqdm(enumerate(img_list), total=len(img_list)):
